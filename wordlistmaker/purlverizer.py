...
def get_endpoints(burp_file, in_scope_domains=[]):
    ...
    Returns:
        dict: A dictionary containing the following keys:
            ...
            'probably_false_positives' (list of str): A list of unique URLs that might be false positives, i.e., not actual endpoints.
            ...
            try:
                if elem.tag == "response" and elem.attrib["base64"] == "true":
                    ...
                    false_positives |= {
                        url
                        for url in urls
                        if not TLDS.search(url) and url not in js_found
                    }
                    all_urls = {
                        url
                        for url in urls
...
def command_line_parser():
    parser = argparse.ArgumentParser(
        description="pu(r)lverizer offline tool. Take a Burp Suite XML file and find potentially interesting stuff such as URLs or API endpoints; or pulverize every URL found in the requests and responses in order to craft target specific wordlists. You can also pass a list of URLs to create the wordlists.",
    ...
    actions_group = parser.add_argument_group(
        "Actions",
        "These options can be used to specify what to do with the provided input. If no action is specified, the script will only print the URLs found in the provided input.",
    )
    ...
    actions_group.add_argument(
        "-a",
        "--api",
        help="Search API endpoints in a Burp XML file",
        action="store_true",
    )
    actions_group.add_argument(
        "-jk",
    ...
    filters_group.add_argument(
        "--in-scope", metavar="", help="Specify one or more in-scope domains (e.g. --in-scope test.com test01.com)", nargs="*"
    )
    filters_group.add_argument(
        "--no-numbers",
        metavar="",
    ...
    filters_group.add_argument(
        "--all-files",
        help="Include results that contain all kind of files (such as jpg, png, etc.)",
        action="store_false",
    )
    filters_group.add_argument(
        "--nonprintable",
    ...
    filters_group.add_argument("-A", "--all", help="Find all (execute all the actions with no filters)", action="store_true")
